...
function ControlPage({ tel, setTel }) {
  ...
  const modes = ["Creep", "Drive", "Precision"];
  const dirs = { up: "FORWARD", down: "REVERSE", left: "LEFT", right: "RIGHT" };

  const handleDir = (dir, down) => {
    setActive(down ? dir : null);
    if (down) {
      setTel(t => ({ ...t, speed: dir === "up" ? speed : dir === "down" ? -speed : speed * 0.4, gear: dir === "up" ? "D" : dir === "down" ? "R" : "D" }));
    } else {
      setTel(t => ({ ...t, speed: 0 }));
    }
  };
  ...
  const DBtn = ({ dir, children }) => (
    <motion.button
      ...
      className={`w-14 h-14 md:w-16 md:h-16 rounded-xl flex items-center justify-center border transition-all
        ${active === dir
          ? "bg-sky-500/40 border-sky-400 text-sky-300 shadow-lg shadow-sky-900/40"
          : "bg-white/6 border-white/12 text-slate-400 hover:bg-white/10 hover:text-white"
        }`}
    >
      {children}
    </motion.button>
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 p-4 overflow-auto">
      {/* D-pad */}
      <Card glow className="flex flex-col items-center gap-4 flex-1">
        <p className="text-[10px] font-mono text-slate-400 tracking-widest self-start">DIRECTIONAL CONTROL</p>
        <div className="grid grid-cols-3 gap-2 mt-2">
          ...
          <motion.button
            whileTap={{ scale: 0.92 }}
            onClick={handleStop}
            className="w-14 h-14 md:w-16 md:h-16 rounded-xl bg-red-900/50 border border-red-700/60 text-red-400 hover:bg-red-700/50 flex items-center justify-center font-mono font-bold text-[10px] tracking-widest"
          >
            STOP
          </motion.button>
        ...
        {active && (
          <div className="text-[10px] font-mono text-sky-400 bg-sky-500/10 border border-sky-500/30 px-4 py-1.5 rounded-lg">
            ▶ {dirs[active]}
          </div>
        )}
        ...
        <div className="w-full mt-2">
          <div className="flex justify-between mb-2">
            <span className="text-[10px] font-mono text-slate-500">SPEED LIMIT</span>
            <span className="text-[10px] font-mono text-sky-400">{speed.toFixed(1)} m/s</span>
          </div>
          <input
          ...
          <div className="flex justify-between text-[10px] font-mono text-slate-600 mt-1">
            <span>0.0</span><span>1.0</span><span>2.0 m/s</span>
          </div>
        </div>
      ...
      <Card className="flex-1 flex flex-col gap-4">
        <p className="text-[10px] font-mono text-slate-400 tracking-widest">VEHICLE STATE</p>
        {[
          { label: "SPEED",   value: `${Math.abs(tel.speed).toFixed(1)} m/s`, color: "#a78bfa" },
          { label: "GEAR",    value: tel.gear,                               color: tel.gear === "R" ? "#ef4444" : tel.gear === "P" ? "#94a3b8" : "#22c55e" },
          { label: "HEADING", value: "N 012°",                               color: "#38bdf8" },
          { label: "MODE",    value: mode.toUpperCase(),                      color: "#f59e0b" },
        ].map(({ label, value, color }) => (
          <div key={label} className="flex flex-col gap-1 p-3 rounded-xl bg-white/4 border border-white/8">
            <span className="text-[10px] font-mono text-slate-500">{label}</span>
...
function SensorsPage({ sensors }) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4 overflow-auto">
      {/* Top-view car + radar */}
      <Card glow className="flex items-center justify-center py-8 relative overflow-hidden" style={{ minHeight: 280 }}>
        <div className="relative flex items-center justify-center">
          {/* Radar rings */}
          {[80, 120, 160].map((r, i) => (
            <div key={r} className="absolute rounded-full border border-sky-500/20 transition-all duration-700"
              style={{ width: r*2, height: r*2, opacity: wave === i ? 0.5 : 0.15 }} />
          ))}
          {/* Car top-view */}
          <svg width="80" height="140" viewBox="0 0 80 140" className="relative z-10">
            <rect x="15" y="20" width="50" height="100" rx="12" fill="#1e3a5f" stroke="#38bdf8" strokeWidth="1.5" />
            <rect x="20" y="30" width="40" height="35" rx="6" fill="#0c2040" stroke="#38bdf8" strokeWidth="0.5" opacity="0.8" />
            <rect x="10" y="25"  width="12" height="20" rx="3" fill="#0a1a30" stroke="#38bdf8" strokeWidth="0.5" />
            <rect x="58" y="25"  width="12" height="20" rx="3" fill="#0a1a30" stroke="#38bdf8" strokeWidth="0.5" />
            <rect x="10" y="95"  width="12" height="20" rx="3" fill="#0a1a30" stroke="#38bdf8" strokeWidth="0.5" />
            <rect x="58" y="95"  width="12" height="20" rx="3" fill="#0a1a30" stroke="#38bdf8" strokeWidth="0.5" />
            {/* Sensor dots */}
            <circle cx="40" cy="22" r="4" fill={sensorColor(sensors.front)} />
            <circle cx="40" cy="118" r="4" fill={sensorColor(sensors.rear)} />
            <circle cx="15" cy="70" r="4" fill={sensorColor(sensors.left)} />
            <circle cx="65" cy="70" r="4" fill={sensorColor(sensors.right)} />
          </svg>
          {/* Sensor value labels */}
          <span className="absolute -top-6 font-mono text-xs font-bold" style={{ color: sensorColor(sensors.front) }}>{sensors.front.toFixed(1)}m</span>
          <span className="absolute -bottom-6 font-mono text-xs font-bold" style={{ color: sensorColor(sensors.rear) }}>{sensors.rear.toFixed(1)}m</span>
          <span className="absolute font-mono text-xs font-bold" style={{ color: sensorColor(sensors.left), left: -52 }}>{sensors.left.toFixed(1)}m</span>
          <span className="absolute font-mono text-xs font-bold" style={{ color: sensorColor(sensors.right), right: -52 }}>{sensors.right.toFixed(1)}m</span>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <SensorCard key={key} label={label} value={sensors[key]} icon={Radio} />
        ))}
      </div>
    </div>
  );
